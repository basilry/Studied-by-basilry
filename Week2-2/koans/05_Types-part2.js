 ...
 */
  ...
  it('원시 자료형을 변수에 할당할 경우, 값 자체의 복사가 일어납니다.', function () {
    let overTwenty = false; // ...왜 false가 되는지 아직도 이해가 안된다.
  ...
  it('원시 자료형 또는 원시 자료형의 데이터를 함수의 인자로 전달할 경우, 값 자체의 복사가 일어납니다.', function () {
    ...
    expect(currentYear).to.equal(2020); // 기존에 선언된 값을 그대로 가져오면 된다.
    expect(after10).to.equal(2030); // after10 함수에 2020이 들어간 값을 가져오면 된다.
  ...
  */
  ...
  it('참조 자료형을 변수에 할당할 경우, 데이터의 주소가 저장됩니다.', function () {
    ...
    */
    ...
    expect(allowedToDrink).to.deep.equal(['hongsik', 'minchul', 'hoyong', 'san']); // or overTwenty
    overTwenty[1] = 'chanyoung'; // ['hongsik', 'chanyoung', 'hoyong', 'san']
