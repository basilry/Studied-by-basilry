...
_.slice = function (arr, start, end) { //arr = [-2, -1, 0, 1] , -2,  -1??????????
  ...
  if (start < 0) _start = Math.max(0, arr.length + start); // Math.max(0, 2) = 2
  if (end < 0) _end = Math.max(0, arr.length + end); // Math.max(0, 3) = 3
...
// test 함수는 각 요소에 반복 적용됩니다.
 ...
 //함수를 파라미터로 받았을 때, 해당 함수의 파라미터를 이프문으로 작성
...
//         // 11 + 5 * 5 = 36; (마지막 작업이므로 최종적으로 36이 리턴됩니다.)
    ...
    _.reduce = function (arr, iteratee, initVal) {
      ...
      _.each(arr, function(ele, idx, arr){
        if(acc === undefined){ //초기 값이 없을 때 
          acc = arr[0]; //초기 값은 배열의 첫 요소 (누적값)
        }else{ //초기 값이 있을 때 
