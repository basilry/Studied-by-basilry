...
buttons.addEventListener('click', function (event) {
  ...
  if (target.matches('button')) {
    ...
    if (action === 'decimal') {
      decimal.textContent = buttonContent;
    }
  ...
  if (target.matches('button')) {
    if (action === 'number') {
      if (display.textContent === '0') {
       ...
       previouNum = display.textContent;
       previousKey = 'number';
      }
    ...
    if (action === 'calculate') { 
      display.textContent = calculate(Number(firstNum), intermediateOperator, Number(display.textContent))
